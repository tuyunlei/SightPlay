...
| Authentication recovery intent     | User cancellation, service failure, and explicit registration preserve their intended recovery actions instead of collapsing into one state        | React integration        | OS authenticator prompt behavior requires compatible-domain device evidence |
...
| Practice state transitions         | Correct/wrong notes, score, streak, settings, curriculum stage/transfer results, song progress, and completion observed through UI/state contracts | stable Chromium          | Physical timing/feel and notated rhythm accuracy are not certified          |
