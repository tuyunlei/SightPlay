...
export const computeScore = (prevScore: number, streak: number) =>
  prevScore + 10 + (streak * 2);
...
export const updateSessionStats = ({ prev, hasMistake, timeDiffMs }: UpdateStatsParams): SessionStats => {
  ...
  const smoothedBpm = prev.totalAttempts === 0
    ? instantaneousBpm
