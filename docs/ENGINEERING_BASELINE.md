...
- Reset coverage thresholds to the rounded-down source-only baseline (73% lines, 72% statements,
  62% branches, 60% functions). Vitest 3+ always excludes test files; the old 80% gate counted test
  code and cannot be preserved on a security-fixed Vitest release.
...
| Vitest / coverage   | 2.1.9 / 2.1.9    | 4.1.10 / 4.1.10   | 454 unit and integration tests + coverage     |
...
454 unit/integration tests with coverage, 56 CI-mode E2E tests, production build, and audit passed.
...
- Source-only coverage is below the historical 80% label. Raise it with product-code tests; do not
  recreate the old inflated metric by broad exclusions or counting test code.
