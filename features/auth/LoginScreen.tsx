...
export function LoginScreen({ initialShowRegister = false, initialInviteCode }: LoginScreenProps) {
  ...
  return (
    <div
      ...
      className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 px-4 py-10"
    >
      <div className="w-full max-w-md">
        <div className="w-full rounded-2xl bg-slate-800/50 p-8 shadow-2xl backdrop-blur-sm">
          <div className="mb-8 flex flex-col items-center gap-4">
            <div className="rounded-full bg-indigo-500/10 p-4">
              <KeyRound className="h-12 w-12 text-indigo-400" />
            </div>
            <div className="text-center">
              <h1 className="text-2xl font-bold text-white">{t.authLoginTitle}</h1>
              <p className="mt-2 text-slate-400">{t.authLoginSubtitle}</p>
            </div>
          </div>
          ...
          {!showRegister && (
            <button
              ...
              className="mt-4 w-full text-center text-sm text-indigo-300 underline decoration-indigo-400/60 underline-offset-4 transition hover:text-indigo-200"
            >
