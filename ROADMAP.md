...
- [ ] 首屏改造：一个"登录"按钮 + 下方"没有账号？用邀请码注册"链接
- [ ] 登录失败后：显示错误提示 + 高亮/展开注册区域
- [ ] 邀请码链接：支持 `/register?code=XXXX-XXXX` 直接进注册流程，邀请码预填
- [ ] 注册流程：填邀请码 → 点按钮发起 passkey 注册

### P4.2 — 中英文本地化

全面支持中英文，覆盖所有页面（含登录注册）。

- [ ] 审计现有 i18n 覆盖范围，列出未本地化的硬编码文本
- [ ] 补全所有页面的 i18n key（登录、注册、错误提示、设置等）
- [ ] 开发流程约束：新增 UI 文本必须走 i18n，不允许硬编码

### P4.3 — 深浅色模式

全面支持深浅色模式，跟随系统自动切换。

- [ ] 设计 token 体系（颜色变量），支持 light/dark 两套
- [ ] 全面适配所有页面（含登录注册、练习、设置等）
- [ ] 跟随系统 `prefers-color-scheme` 自动切换
- [ ] 开发流程约束：新增样式必须使用 design token，不允许硬编码颜色值
